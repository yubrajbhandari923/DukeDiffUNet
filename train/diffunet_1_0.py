...
def eval_step(engine, batchdata):
    ...
    return {"image": image, "y_pred": output, "y": masks}
...
def get_dataloaders(config, aim_logger, rank):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.Lambdad(
                keys=[Keys.LABEL],
                func=functools.partial(threshold_label, num_classes=config.data.num_classes),
            ),
...
def attach_aim_handlers(trainer, val_evaluator, aim_logger, val_loader, rank, config, postprocess=None):
...
def attach_handlers(
    trainer, val_evaluator, net, opt, ema_model, config, aim_logger, val_loader, rank, postprocess=None
):
    """
    ...
    attach_aim_handlers(trainer, val_evaluator, aim_logger, val_loader, rank, config, postprocess=postprocess)
...
def _distributed_run(rank, config):
    ...
    postprocess = transforms.Compose([ transforms.AsDiscreted(
        keys=Keys.PRED, argmax=True, to_onehot=config.data.num_classes
    ), transforms.AsDiscreted(keys="y", to_onehot=config.data.num_classes)])
