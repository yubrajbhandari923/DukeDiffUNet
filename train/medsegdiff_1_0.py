...
def get_dataloaders(config, aim_logger, rank):
    ...
    if rank == 0:
        ...
        val_files = val_files  # Limit to 160 samples for validation
        # val_files = val_files[:16]
        ...
        if config.evaluation.visualize:
            aim_logger.attach(
                ...
                event_name=Events.ITERATION_COMPLETED(
                    ...
                    every= config.evaluation.visualize_num_samples
                ),
                # event_name=Events.ITERATION_COMPLETED
            )
    except Exception as e:
...
def _distributed_run(rank, config):
    ...
    net, opt, diffusion, schduler, ema_model, resume_epoch = build_model(config, rank)
