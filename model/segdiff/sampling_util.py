...
def sampling_major_vote_func(
    ...
    with torch.no_grad():
        for round_index in tqdm(
            ...
            model_kwargs = {
                "conditioned_image": torch.cat(
                    [former_frame_for_feature_extraction] * major_vote_number
                )
            }
            ...
            for i in range(x.shape[0]):
                ...
                out_img = Image.fromarray(
                    x[i][0].detach().cpu().numpy().astype("uint8")
                )
                ...
                out_img = Image.fromarray(
                    (x[i][0].detach().cpu().numpy() - 1).astype(np.uint8)
                )
    ...
    gathered_length_of_data = [
        torch.tensor(1, device=dist_util.dev()) for _ in range(dist.get_world_size())
    ]
    ...
    iou_tensor = torch.tensor(
        miou_list + [torch.tensor(-1)] * (max_len - my_length), device=dist_util.dev()
    )
    f1_tensor = torch.tensor(
        f1_score_list + [torch.tensor(-1)] * (max_len - my_length),
        device=dist_util.dev(),
    )
    wcov_tensor = torch.tensor(
        wcov_list + [torch.tensor(-1)] * (max_len - my_length), device=dist_util.dev()
    )
    boundf_tensor = torch.tensor(
        fbound_list + [torch.tensor(-1)] * (max_len - my_length), device=dist_util.dev()
    )
    gathered_miou = [
        torch.ones_like(iou_tensor) * -1 for _ in range(dist.get_world_size())
    ]
    gathered_f1 = [
        torch.ones_like(f1_tensor) * -1 for _ in range(dist.get_world_size())
    ]
    gathered_wcov = [
        torch.ones_like(wcov_tensor) * -1 for _ in range(dist.get_world_size())
    ]
    gathered_boundf = [
        torch.ones_like(boundf_tensor) * -1 for _ in range(dist.get_world_size())
    ]
